...
sys.path.append("/usr/local/lib/python3.6/site-packages/numpy")
import collections
import os
import pickle
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        :return:
        """
        return ["[PAD]","O","PS","LC","OG","DT","TI","X", "[CLS]","[SEP]"]
...
def _write_base(batch_tokens,id2label,prediction,batch_labels,wf,i):
    ...
    if token!="[PAD]" and token!="[CLS]" and true_l!="X":
        ...
        wf.write(line)
        #logging.info(batch_tokens,true_l,predict)
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        with open(output_eval_file,"w") as wf:
            ...
            # eval save
            eval_res_file = open (os.path.join(FLAGS.output_dir,"eval_res_file.txt"),'w')
            eval_res_file.write(str(p)+'\n'+str(r)+'\n'+str(f)+'\n')
